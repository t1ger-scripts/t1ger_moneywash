...
--- This function triggers the client event `t1ger_moneywash:client:notification` to display a notification.
...
function _API.SendNotification(src, text, type, data)
    if src and tonumber(src) then
        TriggerClientEvent("t1ger_moneywash:client:notification", src, text, type, data)
    else
